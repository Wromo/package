...
- 5874655: Add support for Wromo 0.26.0 attributes
...
- 82b8891: Add HTML hover info, fix Wromo directives producing errors, fix missing children property for JSX based frameworks
...
- 6b6b47a: Remove internal wromo.d.ts files, instead prefer the one provided by Wromo itself
...
- 897ab35: Provide vite client types to Wromo files
...
- 4c93d24: Prevent reading tsconfig in .wromo files
...
- 12b4ed3: Adds support for Wromo.slots typing
...
- e0facf6: Adds an WromoCheck export, to allow running diagnostics programmatically
...
- 1b2afc7: Prevents presence of @types/react from causing false-positive wromo errors
...
- 553969e: Fixes errors when using a tsconfig.json
  ...
  Previously when using a tsconfig.json that had an `include` property, that property would cause diagnostics in wromo files to show JSX related errors. This fixes that issue.
...
- 1958d51: Default Wromo.fetchContent to treat type param as any
...
- f2f7fc8: Removes errors shown when using Wromo.resolve
...
- Adds completion suggestions for Wromo.\* APIs in the component script.
...
- Expose `bin/server.js` as `wromo-ls`

## 0.5.0-next.0

- Moved to scoped `@wromojs/language-server` package
